...
class Base
{
	protected void show(int a)			//show() with 1 parameter
	{
		System.out.println("\nIn Base show()");
	}
}
...
{
	public void show()				//show() with 0 parameter, Hence no overriding bcz, header of Base show() & Derive show() is different.
	{
		System.out.println("\nIn Derive show()");
	}
}
...
class Pro15_poly
{
	public static void main(String args[])
	{
		...
		ref.show();				/*Here, compiler will bind function call at compile time depending upon datatype of reference variable. & type of
								  ...
								  no parameter in Base class only & as function show() with 0 para is not available in Base class, it generates
								  error for both statements. */
		ref = new Derive();		
		ref.show();				
	}
}